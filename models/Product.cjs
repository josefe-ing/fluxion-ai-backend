...
class Product {
  ...
  static async getAll(tenantSchema, options = {}) {
    ...
    try {
      ...
      // Construir query con schema del tenant
      let sql = `
        SELECT 
   ...
   * @returns {Promise<Object|null>} Producto encontrado o null
   */
  static async getBySku(tenantSchema, sku) {
    if (!tenantSchema) {
      throw new Error('tenantSchema es requerido para operaciones multi-tenant');
    }
  ...
  static async create(tenantSchema, productData) {
    ...
    try {
      ...
      const existing = await this.getBySku(tenantSchema, sku);
      if (existing) {
   ...
   * @returns {Promise<Object>} Producto actualizado
   */
  static async updateStock(tenantSchema, id, newStock, reason = 'manual_adjustment') {
    if (!tenantSchema) {
      throw new Error('tenantSchema es requerido para operaciones multi-tenant');
    }
